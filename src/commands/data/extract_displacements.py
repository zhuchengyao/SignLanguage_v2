...
from src.pose_normalizer import normalize_sequence_xy, normalize_sequence_xy_global
...
def main():
    ...
    parser.add_argument('--norm_mode', type=str, default='scale_only', choices=['scale_only', 'global'], help='normalization mode: scale_only (no translate/rotate/mirror) or global (translate+rotate+mirror+scale)')
        ...
        for sp, sid, d in samples:
            ...
            if args.save_norm:
                ...
                if args.norm_mode == 'global':
                    seq_norm, info = normalize_sequence_xy_global(seq_xy)
                    scale_info = {
                        'norm_mode': 'global',
                        'theta0': float(info.get('theta0', 0.0)),
                        'scale_denom0': float(info.get('scale_denom0', 1.0)),
                        'mirrored_x': bool(info.get('mirrored_x', False)),
                    }
                else:
                    # scale_only: compute a single scalar denom from first frame without translate/rotate/mirror
                    p0_xy = seq_xy[0].astype(np.float32)  # (50,2)
                    pelvis = p0_xy[0]
                    chest = p0_xy[1]
                    r_sh = p0_xy[2]
                    l_sh = p0_xy[5]
                    sh_width = float(np.linalg.norm(l_sh - r_sh))
                    torso_len = float(np.linalg.norm(chest - pelvis))
                    denom = max(sh_width, torso_len, 1e-6)
                    seq_norm = (seq_xy / denom).astype(np.float32)
                    scale_info = {
                        'norm_mode': 'scale_only',
                        'scale_denom0': float(denom),
                    }
