"""Evaluate Motion VAE reconstruction quality."""
...
@torch.no_grad()
def evaluate(checkpoint_path: str, split: str = 'dev', output_dir: str = './outputs/eval_vae', num_samples: int = 8):
    ...
    dataset = ASLPoseDataset(
        ...
        extern_std=cfg.std.cpu().numpy() if cfg.std is not None else None,
        cache_in_memory=True,
        cache_max_items=getattr(cfg, 'dataset_cache_max_items', 1024),
    )

    from torch.utils.data import DataLoader
    loader = DataLoader(dataset, batch_size=1, shuffle=False, collate_fn=collate_pose_batch, num_workers=0)
    ...
    for batch in tqdm(loader, total=min(len(loader), num_samples)):
        ...
        metrics.append({'mse_xy': mse_xy.item(), 'kl': out['kl_loss'].item()})
    ...
    if metrics:
        ...
        kl_vals = [m['kl'] for m in metrics]
        print(f"Eval {split}: MSE(xy) mean={np.mean(mse_vals):.4f} std={np.std(mse_vals):.4f} "
              f"KL mean={np.mean(kl_vals):.4f} n={len(mse_vals)}")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--checkpoint', type=str, default='./checkpoints/vae_model.pth')
    parser.add_argument('--split', type=str, default='dev')
    parser.add_argument('--output_dir', type=str, default='./outputs/eval_vae')
